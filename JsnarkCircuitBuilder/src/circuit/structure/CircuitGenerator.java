...
public abstract class CircuitGenerator {
	...
	public final void generateCircuit() {
		...
		System.out.println("test Running Circuit Generator for < " + circuitName + " >");
	...
	public void runLibsnarkVerify(int n) {

		try {
			Process p;
			p = Runtime.getRuntime()
					.exec(new String[] { Config.LIBSNARK_EXEC, circuitName + ".arith", circuitName + ".in" , "verify", Integer.toString(n)});
			p.waitFor();
			System.out.println(
					"\n-----------------------------------RUNNING VERIFY -----------------------------------------");
			String line = "";
