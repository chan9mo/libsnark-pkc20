...
witness<ppT> kzg_witness(commitkey<ppT> &ck, libff::Fr_vector<ppT> &poly, libff::Fr<ppT> &point, int t)
{
    ...
    /* Evaluate Polynomial */
    libff::enter_block("Evaluate Polynomial + Constant Update");
    ...
    libff::leave_block("Evaluate Polynomial + Constant Update");
    ...
    /* Output as a witness */
    witness<ppT> wit = witness<ppT>(std::move(point), std::move(eval), std::move(psi), std::move(w1), std::move(w2));
    return wit;
}
